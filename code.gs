...
/**
 * نقطة الدخول لطلبات POST القادمة من الصفحة المستضافة خارج Apps Script
 * (مثلًا من Cloudflare Pages) عبر fetch().
 * الجسم المتوقع: { action: "save", data: {...}, attachments: [...] }
 *              أو { action: "query", nationalId: "..." }
 */
function doPost(e) {
  let payload;
  try {
...
/**
 * تهيئة ورقة البيانات وإنشاء مجلد المرفقات عند الحاجة.
 */
function setup() {
...
/**
 * حفظ أو تحديث طلب.
 * data = بيانات النموذج، وattachments = مصفوفة {name, mimeType, base64}
 */
